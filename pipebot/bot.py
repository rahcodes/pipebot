...
class PipeBot:
    ...
    def rotate_face(self, steps: int, direction: Literal["clockwise", "anti-clockwise"], step_delay: float = 0.001) -> None:
        """Rotates the face in the given direction and number of steps (512 = 360deg)"""

        assert (self._face_pins), "Face pins are not set."
        assert (direction in ["clockwise", "anti-clockwise"]), "Direction must be clockwise or anti-clockwise."

        signals = STEPPER_SIGNALS if direction == "anti-clockwise" else list(map(lambda sig: sig[::-1], STEPPER_SIGNALS))
    ...
    def init_accelerometer(self) -> None:
        ...
        if hex_address in output.decode():
            ...
            raise Exception("Accelerometer not found. Check connection: SDA->GPIO2 and SCL->GPIO3")
    ...
    def move_robot(self, duration: float, direction: Literal["forward", "backward"]) -> None:
        """Moves the robot forward/backward for given duration (seconds) and direction"""

        assert (self._tyre_pins), "Tyre pins are not set."
        assert (direction in ["forward", "backward"]), f"Invalid direction to move '{direction}'"
